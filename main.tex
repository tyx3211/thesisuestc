...
\setheadercontent{信软学院进阶式挑战性综合项目III中期报告}
...
\subsection{有序列表测试（enumerate）}
本文主要包含以下几个部分：
\begin{enumerate}
  \item 第一章介绍了研究背景和意义
  \item 第二章阐述了时域积分方程基础理论
  \item 第三章讨论了算法实现的关键技术
  \item 第四章给出了数值实验结果
  \item 第五章总结了全文并展望未来研究方向
\end{enumerate}

\subsection{无序列表测试（itemize）}
本研究采用的方法包括：
\begin{itemize}
  \item 时间步进算法（MOT）用于求解时域积分方程
  \item 平面波时域算法（PWTD）加速矩阵向量乘积
  \item RWG基函数进行空间离散
  \item 三角脉冲函数进行时间离散
\end{itemize}

\subsection{描述列表测试（description）}
主要符号说明：
\begin{description}
  \item[MOT] Marching-on-in-time，时间步进算法
  \item[PWTD] Plane Wave Time-domain，平面波时域算法
  \item[RWG] Rao-Wilton-Glisson，一种空间基函数
  \item[TDIE] Time-domain Integral Equation，时域积分方程
\end{description}
...
\thesiscodefile[H][示范代码块。]{demo.cpp}
